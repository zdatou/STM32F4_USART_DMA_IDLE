...
int PTPd_Init(void)
{
    ...
    rtOpts.clockQuality.clockAccuracy = DEFAULT_CLOCK_ACCURACY;		 //0xFE 精度未知   时钟精度，取值越低精确度越高
    rtOpts.clockQuality.clockClass = DEFAULT_CLOCK_CLASS;	   //248  默认,如果无其他clockClass定义适用,则使用clockClass
    rtOpts.clockQuality.offsetScaledLogVariance = DEFAULT_CLOCK_VARIANCE; /* 7.6.3.3 */		//5000 时钟稳定性
    rtOpts.priority1 = DEFAULT_PRIORITY1;	 //248  时钟优先级1，支持用户配置，取值范围是0～255，取值越小优先级越高
    rtOpts.priority2 = DEFAULT_PRIORITY2;	 //248  时钟优先级2，支持用户配置，取值范围是0～255，取值越小优先级越高
